...
namespace FIMSpace.FTail
{
    ...
    public class FTailAnimator_Editor_Base : Editor
    {
        ...
        protected virtual void DrawPhysicalParametersGUI(FTail_AnimatorBase tail)
        {
            ...
            if (!Application.isPlaying)
            {
                if (tail.UseCollision)
                {
                    ...
                    if (tail.CollisionSpace == FTail_AnimatorBase.ECollisionSpace.World_Slow)
                    {
                        ...
                        bool colTyp = tail.CollidersType != 0;
                        colTyp = EditorGUILayout.Toggle(new GUIContent("Capsule Collision", "Capsule collision for world space can give you more precise detection when you have lesser bones"), colTyp);
                        if (colTyp) tail.CollidersType = 1; else tail.CollidersType = 0;
                    }
                    else
                    {
                    ...
                    //    tail.CollidersType = EditorGUILayout.IntSlider(new GUIContent("Denser Colliders"), tail.CollidersType, 0, 2);
                    //}
            ...
            else // In Playmode
            {
                if (tail.UseCollision)
                {
                    ...
                    else
                    {
                        ...
                        EditorGUILayout.PropertyField(sp_CollidersType);
            ...
            {
                if (tail.OptimizeWithMesh == null)
                {
                    tail.OptimizeWithMesh = tail.transform.GetComponent<Renderer>();
                    if (!tail.OptimizeWithMesh) tail.OptimizeWithMesh = tail.transform.GetComponentInChildren<Renderer>();
                    if (!tail.OptimizeWithMesh) if (tail.transform.parent != null) tail.OptimizeWithMesh = tail.transform.parent.GetComponentInChildren<Renderer>();
                    if (!tail.OptimizeWithMesh) if (tail.transform.parent != null) if (tail.transform.parent.parent != null) tail.OptimizeWithMesh = tail.transform.parent.parent.GetComponentInChildren<Renderer>();
                    if (!tail.OptimizeWithMesh) if (tail.transform.parent != null) if (tail.transform.parent.parent != null) if (tail.transform.parent.parent.parent != null) tail.OptimizeWithMesh = tail.transform.parent.parent.parent.GetComponentInChildren<Renderer>();
                }
            }
