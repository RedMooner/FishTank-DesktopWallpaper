...
namespace FIMSpace.FTail
{
    ...
    public abstract class FTail_AnimatorBase : MonoBehaviour
    {
        ...
        {
            if (Application.isPlaying) if (wasRunning) if (!initialized) Init();
        }
        ...
        public virtual void CalculateOffsets()
        {
            ...
            {
                if (!OptimizeWithMesh.isVisible) return;
            }
        ...
        private void AddColliders()
        {
            if (CollisionSpace == ECollisionSpace.World_Slow)
            {
                ...
                if (CollidersType != 0)
                {
                    for (int i = 1; i < TailTransforms.Count - 1; i++)
                    {
                        CapsuleCollider caps = TailTransforms[i].gameObject.AddComponent<CapsuleCollider>();
                        ...
                        caps.radius = GetColliderSphereRadiusFor(TailTransforms, i);
                        caps.direction = 2;
                        caps.height = (TailTransforms[i].position - TailTransforms[i + 1].position).magnitude * 2f - caps.radius;
                        caps.center = TailTransforms[i].InverseTransformPoint(Vector3.Lerp(TailTransforms[i].position, TailTransforms[i + 1].position, 0.5f));

                        proceduralPoints[i].ColliderRadius = caps.radius;
                ...
                else
                {
                    for (int i = 1; i < TailTransforms.Count; i++)
                    {
                        SphereCollider s = TailTransforms[i].gameObject.AddComponent<SphereCollider>();
                        FTail_CollisionHelper tcol = TailTransforms[i].gameObject.AddComponent<FTail_CollisionHelper>().Init(CollidersAddRigidbody, RigidbodyMass);
                        tcol.TailCollider = s;
                        tcol.Index = i;
                        tcol.ParentTail = this;
                        s.radius = GetColliderSphereRadiusFor(TailTransforms, i);
                        proceduralPoints[i].ColliderRadius = s.radius;
                        proceduralPoints[i].CollisionHelper = tcol;
                    }
                }
            }
        ...
        protected float GetColliderSphereRadiusFor(int i)
        {
            ...
            if (i >= TailTransforms.Count) return refDistance;

            if (TailTransforms.Count > 1)
            {
                refDistance = Vector3.Distance(TailTransforms[1].position, TailTransforms[0].position);
            }

            //if (i == 0)
            //    targetDistance = refDistance;
            //else
            //    targetDistance = Vector3.Distance(TailTransforms[i - 1].position, TailTransforms[i].position);

            float singleScale = refDistance;
            if (i != 0) singleScale = Mathf.Lerp(refDistance, Vector3.Distance(TailTransforms[i - 1].position, TailTransforms[i].position) * 0.5f, DifferenceScaleFactor);
        ...
        protected virtual void OnDrawGizmosSelected()
        {
            ...
            if (!Application.isPlaying)
            {
                if (UseCollision)
                {
                    ...
                    for (int i = 1; i < editorGizmoTailList.Count; i++)
                    {
                        ...
                        Matrix4x4 curr = Matrix4x4.TRS(editorGizmoTailList[i].position, editorGizmoTailList[i].rotation, editorGizmoTailList[i].lossyScale);
                        float radius = GetColliderSphereRadiusFor(editorGizmoTailList, i);
                        ...
                        if (CollisionSpace == ECollisionSpace.World_Slow)
                            if (CollidersType != 0)
                            {
                                Matrix4x4 pre = Matrix4x4.TRS(editorGizmoTailList[i - 1].position, editorGizmoTailList[i - 1].rotation, editorGizmoTailList[i - 1].lossyScale);
                                Gizmos.DrawLine(pre.MultiplyPoint(Vector3.up * radius), curr.MultiplyPoint(Vector3.up * radius));
                                Gizmos.DrawLine(pre.MultiplyPoint(-Vector3.up * radius), curr.MultiplyPoint(-Vector3.up * radius));
                                Gizmos.DrawLine(pre.MultiplyPoint(Vector3.right * radius), curr.MultiplyPoint(Vector3.right * radius));
                                Gizmos.DrawLine(pre.MultiplyPoint(Vector3.left * radius), curr.MultiplyPoint(Vector3.left * radius));
                            }
                    }
            ...
            else
            {
                if (UseCollision)
                {
                    ...
                    for (int i = 1; i < TailTransforms.Count; i++)
                    {
                        if (TailTransforms[i] == null) continue;
                        Matrix4x4 curr = Matrix4x4.TRS(TailTransforms[i].position, TailTransforms[i].rotation, TailTransforms[i].lossyScale);
                        float radius = proceduralPoints[i].ColliderRadius;

                        if (CollidersType != 0)
                        {
                            if (CollisionSpace == ECollisionSpace.World_Slow)
                            {
                                Gizmos.color = Color.HSVToRGB((float)i / (float)TailTransforms.Count, 0.9f, 0.9f) * new Color(1f, 1f, 1f, 0.7f);

                                Matrix4x4 pre = Matrix4x4.TRS(TailTransforms[i - 1].position, TailTransforms[i - 1].rotation, TailTransforms[i - 1].lossyScale);
                                Gizmos.DrawLine(pre.MultiplyPoint(Vector3.up * radius), curr.MultiplyPoint(Vector3.up * radius));
                                Gizmos.DrawLine(pre.MultiplyPoint(-Vector3.up * radius), curr.MultiplyPoint(-Vector3.up * radius));
                                Gizmos.DrawLine(pre.MultiplyPoint(Vector3.right * radius), curr.MultiplyPoint(Vector3.right * radius));
                                Gizmos.DrawLine(pre.MultiplyPoint(Vector3.left * radius), curr.MultiplyPoint(Vector3.left * radius));
